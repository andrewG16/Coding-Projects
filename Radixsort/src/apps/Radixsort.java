...
public class Radixsort {
	...
	throws IOException {
		...
		Node ptr;
		masterListRear = new Node (sc.next(), masterListRear);
		ptr = masterListRear;
		
		while(sc.hasNext() == true){
			ptr.next = new Node (sc.next(), ptr.next);
			ptr = ptr.next;
		}
		
		if(sc.hasNext() == false && ptr.next == null){
			ptr.next = masterListRear;
		}
	...
	public void scatter(int pass) {
		...
		Node <String> ptrT = masterListRear;
		Node ptr;
		int totalNodes = 0;
		int i = 0;
		...
		do{
			...
			ptrT = ptrT.next;
			totalNodes++;
		}while (prev.next != masterListRear);
		...
		while( i < totalNodes){
			...
			try{
				 temp = java.lang.Character.digit(ptrT.data.charAt(ptrT.data.length()-1-pass), radix);	
			}
			...
			if(ptr == null){
				buckets[temp] = ptrT;
				//buckets[temp].next = buckets[temp];
				//buckets[temp] = new Node(ptrT.data, buckets[temp]);
			}
			else{
				
				while(ptr.next != buckets[temp]){
					ptr = ptr.next;
				}	
				ptr.next = ptrT;
				//ptr = ptr.next;
				//ptr.next = buckets[temp];
				
				//ptr.next = new Node(ptrT.data, ptr.next);
			}
			...
			else if(ptr.next != buckets[temp]){
				ptr.next.next = buckets[temp];
			}
			else if (ptr.next == buckets[temp]){
				ptr.next.next = buckets[temp];
			}
	...
	public void gather() {
		...
		for(int i = 0; i <buckets.length; i++){
			...
			//while(ptr.next != buckets[i] || ptr.next.next != buckets[i]){
				ptr2 = ptr;
				ptr = ptr.next;
			}
			...
			if(masterListRear == null){
				...
				ptrT = masterListRear.next;
				prev = masterListRear;
				
				while(ptrT.next != masterListRear){
					prev = ptrT;
					ptrT = ptrT.next;
				}
				ptrT.next = ptr.next;
